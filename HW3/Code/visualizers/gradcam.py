...
class GradCam:
    def guided_backprop(self, X_tensor, y_tensor, gc_model):
        ...
        the input images.
        Note:
        1) Sum over all the prediciton to obtain a scalar and do a single backprop over 
	    the scalar 
        2) Please use torch.transpose instead of torch.view or torch.permute to reshape 
    ...
    def grad_cam(self, X_tensor, y_tensor, gc_model):
        ...
        Note: Sum over all the prediciton to obtain a scalar and do a single backprop over 
	    the scalar 
        """
